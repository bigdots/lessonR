import React, { useEffect, useRef } from 'react'
// import * as echarts from 'echarts'

// 引入 echarts 核心模块，核心模块提供了 echarts 使用必须要的接口。
import * as echarts from 'echarts/core'
// 引入柱状图图表，图表后缀都为 Chart
import { LineChart, PieChart } from 'echarts/charts'
// 引入提示框，标题，直角坐标系，数据集，内置数据转换器组件，组件后缀都为 Component
import {
  TitleComponent,
  TooltipComponent,
  GridComponent,
  DatasetComponent,
  TransformComponent,
} from 'echarts/components'
// 标签自动布局、全局过渡动画等特性
import { LabelLayout, UniversalTransition } from 'echarts/features'
// 引入 Canvas 渲染器，注意引入 CanvasRenderer 或者 SVGRenderer 是必须的一步
import { CanvasRenderer } from 'echarts/renderers'

// 注册必须的组件
echarts.use([
  LineChart,
  PieChart,
  TitleComponent,
  TooltipComponent,
  GridComponent,
  DatasetComponent,
  TransformComponent,
  LabelLayout,
  UniversalTransition,
  CanvasRenderer,
])

function ChartComponent(props: any) {
  const { option } = props
  const chart = useRef<any>(null)
  useEffect(() => {
    if (!chartWrapper.current) {
      return
    }
    if (!chart.current) {
      chart.current = echarts.init(chartWrapper.current) //初始化ECharts
      window.addEventListener('resize', function () {
        chart.current.resize()
      })
    }
  }, [props])

  useEffect(() => {
    //每次当option变化时，再次setOptions
    if (option) {
      chart.current.setOption(option)
    }
  }, [option])

  const style: React.CSSProperties = {
    width: '100%',
    height: '100%',
  }

  const chartWrapper = useRef<HTMLDivElement>(null)

  return <div style={style} ref={chartWrapper} />
}

export class Pieoption {
  constructor(
    val1: number,
    val2: number,
    title: string,
    labelFormatter: string
  ) {
    this.series[0].data = [
      {
        value: val1,
        name: '已上',
      },
      {
        value: val2,
        name: '未上',
      },
    ]

    this.title.text = title

    this.series[0].label.formatter = labelFormatter
  }

  public title = {
    text: '圆环图',
    left: 'center',
    top: 'center',
  }

  public series = [
    {
      type: 'pie',
      radius: ['40%', '70%'],
      avoidLabelOverlap: false,
      label: {
        show: true,
        position: 'outside',
        formatter: '{b}: {c}h',
      },
      labelLine: {
        show: true,
      },
      data: [
        {
          value: 335,
          name: '已上',
        },
        {
          value: 234,
          name: '未上',
        },
      ],
    },
  ]
}

export class LineOptions {
  constructor(xVal: string[], yVal: number[]) {
    this.xAxis.data = xVal
    this.series[0].data = yVal
  }

  public xAxis = {
    type: 'category',
    data: ['A', 'B', 'C'],
  }

  public yAxis = {
    type: 'value',
  }

  public series = [
    {
      data: [120, 200, 150],
      type: 'line',
      label: {
        show: true,
        position: 'top',
      },
    },
  ]
}

export default ChartComponent
